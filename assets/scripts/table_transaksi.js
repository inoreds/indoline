var TableTransaksi = function () {
    ...
    var handleTable = function () {
        ...
        var tableWrapper = $("#table-transaksi_wrapper");
        ...
        $('#table-transaksi_new').click(function (e) {
            e.preventDefault();
        ...
        $("#hargaBeli").keyup(function(){
            var qty = $('#jumlahPermintaan').val().replace(/,/g , "");
            var harga = $('#hargaBeli').val().replace(/,/g , "");
            var total = Math.round(qty * harga);
            $('#totalPenjualan').val(total.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"));
             
        });

        $("#jumlahPermintaan").keyup(function(){
            var qty = $('#jumlahPermintaan').val().replace(/,/g , "");
            var harga = $('#hargaBeli').val().replace(/,/g , "");
            var total = Math.round(qty * harga);
            $('#totalPenjualan').val(total.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"));
        ...
        $('#simpanDetail').click( function() { 
            
            var aiNew = oTable.fnAddData(['', '', '', '','','','']);
            var nRow = oTable.fnGetNodes(aiNew[0]);
            var newString = new Array();
            oTable.fnUpdate($('#itemInvoice').val(), nRow, 0, false);
            oTable.fnUpdate($('#descInvoice').val(), nRow, 1, false);
            oTable.fnUpdate($('#qtyInvoice').val(), nRow, 2, false);
            oTable.fnUpdate($('#uomInvoice').val(), nRow, 3, false);
            oTable.fnUpdate($('#priceInvoice').val(), nRow, 4, false);
            oTable.fnUpdate($('#totalInvoice').val(), nRow, 5, false);
