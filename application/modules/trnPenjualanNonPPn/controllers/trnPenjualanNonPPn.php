...
class trnPenjualanNonPPn extends CI_Controller {
	...
	function index()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$this->load->view('trnPenjualanNonPPn/content', $d);
			$this->load->view('trnPenjualanNonPPn/modal_PO', $d);
			$this->load->view('trnPenjualanNonPPn/modal_MBA', $d);	
			$this->load->view('trnPenjualanNonPPn/modal_MBT', $d);	
	...
	function invoice()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			if ($JenisPPN == "ppn" AND $MBAOAT=="MBA")
			{
				$this->load->view('trnPenjualanNonPPn/invoicePPn', $d);
			}
			elseif ($JenisPPN == "ppn" AND $MBAOAT == "OAT")
			{
				$this->load->view('trnPenjualanNonPPn/invoicePPnOAT', $d);
			}
			else
			{
				$this->load->view('trnPenjualanNonPPn/invoice', $d);
			}
	...
	function kwitansi()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			if ($row) 
			{
				...
				$tanggal = date("m", strtotime($row->TglAwalSupply));
			...
			if ($JenisPPn == "ppn") 
			{
				$this->load->view('trnPenjualanNonPPn/kwitansiPPn', $d);	

			} 
			else 
			{
				$this->load->view('trnPenjualanNonPPn/kwitansi', $d);		
			}
			$this->load->view('dashboard/bg_footer', $d);
	...
	{ var_dump($_POST);
		...
		$arrItemlokasi = explode(';' , $_POST['lokasi']);
		...
		for($i=0;$i<$result;$i++)
		{
			$total = $total + $arrItemSubTotal[$i];
		}	

		$noPoCustomer	 = $this->kode_inc($kode);
		
		$dt['noPO'] = $_POST['noPO'];
		$dt['noInvoice'] = $_POST['noInvoice'];
		$dt['tglPO'] = $_POST['tglPO'];
		$dt['idCustomer'] = $_POST['idCustomer'];
		$dt['bank'] = $_POST['bank'];
	...
	function simpanMBA()
	{
		...
		$kartu['TglTransaksi'] =  $_POST['dateMBA'];
	...
	function printOut_MBA()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$this->load->view('dashboard/bg_header', $d);
			$this->load->view('dashboard/bg_navigation', $d);
			$this->load->view('trnPenjualanNonPPn/printOut_MBA', $d);
			$this->load->view('dashboard/bg_footer', $d);
	...
	function printOut_MBT()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$this->load->view('dashboard/bg_header', $d);
			$this->load->view('dashboard/bg_navigation', $d);
			$this->load->view('trnPenjualanNonPPn/printOut_MBT', $d);
			$this->load->view('dashboard/bg_footer', $d);
