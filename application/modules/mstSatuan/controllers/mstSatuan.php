...
class mstSatuan extends CI_Controller {
	...
	function index()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$this->load->view('mstSatuan/content', $d);			
	...
	function simpan()
	{
		...
		$dt['namaSatuan'] = $_POST['namaSatuan'];
		$st = $_POST['st'];
		
		if($st=="tambah")
		{
			$dt['idSatuan'] = $this->kode_inc($kode);
			$this->db->insert("MstSatuan",$dt);
			$result =1;
		...
		{
			$id['idSatuan'] = $st;
			$this->db->update("MstSatuan",$dt,$id);
			$result=1;
		}	
	...
	function printOut()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$this->load->view('dashboard/bg_header', $d);
			$this->load->view('dashboard/bg_navigation', $d);
			$this->load->view('mstSatuan/printOut', $d);			
			$this->load->view('dashboard/bg_footer', $d);
	...
	function hapus()
	{
		$id = $_GET['id'];
		$where['idSatuan'] = $id;
		$this->db->delete("MstSatuan",$where);
	...
	{
		$maxid = 0;
		$row = $this->db->query('SELECT MAX(right(idSatuan,2)) AS maxid FROM MstSatuan')->row();
		if ($row) {
		    $maxid = $row->maxid; 
		}
		return $maxid;
