...
class app_load_data_table extends CI_Model {
	...
	public function getAllData()
    {
	    ...
	    $get  = $this->db->query("Select *, pp.POPembelian as POPembelian_1 From PembelianPertamina pp
	    	 						...
	    	 						left OUTER JOIN reciept_for_bunker r on pp.POPembelian = r.POPembelian 
	    	 						left outer join invoicepertamina i on pp.popembelian = i.popembelian
	    	 						left join mstpbbkb mp on pp.idPBBKB = mp.idPBBKB
	...
	public function getAllDataRecieptForBunker()
	{
		$get = $this->db->query("Select * from reciept_for_bunker");
		return $get->result();
	}
	public function getAllDataRecieptForBunker_detil()
	{
		$get = $this->db->query("select *, d.POPembelian as POPembelian_1, d.NoResi as NoResi_1 from reciept_for_bunker d left outer join dopertamina do on d.popembelian=do.popembelian and d.noinvoice=do.noinvoice and d.detilno = do.detilNo");
		return $get->result();
	}
    ...
    {
    	$get = $this->db->query("Select * from PembelianPertamina p, mstpbbkb mp, MstSupplier s, BarangBBM b, MstKapal k where mp.idPBBKB = p.idPBBKB and k.IdVessel = p.IdVessel and p.IdSupplier = s.IdSupplier and p.IdBBM = b.IdBBM and POPembelian='$noPO'");
